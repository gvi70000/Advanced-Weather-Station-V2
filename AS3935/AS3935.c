 ...
 * Register addresses
 * ---------------------------------------------------------------------- */
#define AS3935_REG_AFE_GAIN         0x00U
#define AS3935_REG_THRESHOLD        0x01U
#define AS3935_REG_LIGHTNING        0x02U
#define AS3935_REG_INT_MASK_ANT     0x03U
#define AS3935_REG_ENERGY_LSB       0x04U  /* S_LIG_L  — burst start */
#define AS3935_REG_ENERGY_MSB       0x05U  /* S_LIG_M  */
#define AS3935_REG_ENERGY_MMSB      0x06U  /* S_LIG_MM [4:0] */
#define AS3935_REG_DISTANCE         0x07U  /* DISTANCE [5:0] */
#define AS3935_REG_FREQ_DISP_IRQ    0x08U
#define AS3935_REG_CALIB_TRCO       0x3AU
#define AS3935_REG_CALIB_SRCO       0x3BU
#define AS3935_REG_PRESET_DEFAULT   0x3CU
#define AS3935_REG_CALIB_RCO        0x3DU
...
static uint32_t AS3935_MeasureFrequencyHz(void) {
    ...
    const uint32_t timerClock = AS3935_GetTimerClockHz() /
                                (htim2.Init.Prescaler + 1U);
    const uint32_t N = 500U;   /* number of complete periods to gate       */
    const uint32_t TIMEOUT_TICKS = timerClock / 2U; /* 500 ms max wait     */

    uint32_t t0 = 0U, tN = 0U;
    uint32_t elapsed = 0U;
    ...
    for (edgeCount = 0U; edgeCount < N; edgeCount++) {
        elapsed = 0U;
        while (__HAL_TIM_GET_FLAG(&htim2, TIM_FLAG_CC4) == RESET) {
            if (++elapsed > TIMEOUT_TICKS) {
                HAL_TIM_IC_Stop(&htim2, TIM_CHANNEL_4);
    ...
    uint32_t span = (tN >= t0) ? (tN - t0)
                               : (0xFFFFFFFFU - t0 + tN + 1U);
...
HAL_StatusTypeDef AS3935_Init(void) {
    ...
    if (AS3935_WriteReg(AS3935_REG_THRESHOLD,
                        (uint8_t *)&AS3935_Sensor.NOISE.Val.Value, 1) != HAL_OK)
        return HAL_ERROR;
    ...
    AS3935_Sensor.STATISTICS.Val.Value = 0x80U; /* reserved bit pre-set */
    ...
    if (AS3935_WriteReg(AS3935_REG_LIGHTNING,
        ...
        return HAL_ERROR;
    AS3935_Sensor.STATISTICS.Val.BitField.CL_STAT = 0U;
    if (AS3935_WriteReg(AS3935_REG_LIGHTNING,
                        (uint8_t *)&AS3935_Sensor.STATISTICS.Val.Value, 1) != HAL_OK)
        return HAL_ERROR;
    ...
    AS3935_Sensor.INT_FREQ.Val.Value = 0U;
    ...
    uint8_t irqCfg = AS3935_Sensor.INT_FREQ.Val.Value & 0xF0U; /* mask read-only INT bits */
    ...
    if (AS3935_ReadTRCOCalibStatus() != HAL_OK) return HAL_ERROR;
    if (AS3935_ReadSRCOCalibStatus() != HAL_OK) return HAL_ERROR;

    if (AS3935_Sensor.TRCO.Val.BitField.TRCO_CALIB_NOK ||
        AS3935_Sensor.SRCO.Val.BitField.SRCO_CALIB_NOK) {
        DEBUG("AS3935: Oscillator calibration FAILED "
              "(TRCO=0x%02X SRCO=0x%02X)\n",
...
HAL_StatusTypeDef AS3935_ReadAllRegisters(void) {
    ...
    if (AS3935_ReadRegs(AS3935_REG_CALIB_TRCO,
                        &AS3935_Sensor.TRCO.Val.Value, 1) != HAL_OK)
        return HAL_ERROR;
    if (AS3935_ReadRegs(AS3935_REG_CALIB_SRCO,
                        &AS3935_Sensor.SRCO.Val.Value, 1) != HAL_OK)
        return HAL_ERROR;
...
HAL_StatusTypeDef AS3935_ApplySensitivityProfile(AS3935_Profile_t profile) {
    ...
    if (AS3935_SetGain(p->gain)               != HAL_OK) return HAL_ERROR;
    if (AS3935_SetNoiseFloor(p->nf)           != HAL_OK) return HAL_ERROR;
    if (AS3935_SetWatchdog(p->wdth)           != HAL_OK) return HAL_ERROR;
    if (AS3935_SetSpikeRejection(p->srej)     != HAL_OK) return HAL_ERROR;
...
AS3935_TuneCap_t AS3935_TuneAntenna(void) {
    ...
    const uint32_t TARGET_SRCO = 1100000U;  /* System RC oscillator       */
